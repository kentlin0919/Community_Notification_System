...
Community Notification System 是以 Gin + GORM 打造的 RESTful 服務，提供完整的多社區物業管理解決方案。系統涵蓋使用者管理、物業後台、公設預約、訊息中心、報修管理、訪客管理等核心功能，並內建 JWT、Cookie Session、CORS 等安全機制。

## 專案亮點

- **模組化架構**：採 `controller / repository / model` 分層，並依業務功能（物業、公設、訪客...）拆分模組，降低耦合度。
- **自動化維運**：透過環境變數配置 PostgreSQL 連線，啟動時自動檢查並建立必要資料表與關聯。
- **安全性**：全域 JWT 驗證、CORS 策略、密碼雜湊與 Session 管理，可快速擴充額外安全策略。
- **文件友善**：Swagger (`/swagger/index.html`) 自動生成 API 文件，方便檢視與測試。
- **容器化支援**：內建 Docker 與 Docker Compose 配置，支援熱重載開發。
...
- **3.1 使用者管理 (`user`)**：處理住戶、管理員的註冊、登入、刪除、與權限驗證。
- **3.2 物業後台 (`property`)**：物業管理人員專用的後台操作邏輯，包含社區公告與管理作業。
- **3.3/3.4 公設管理與預約 (`facility`)**：提供公設項目的新增、維護，以及住戶端的公設預約與點數扣除功能。
- **3.5 訊息中心 (`message`)**：處理信件通知、包裹領取、外送拍照存證與社區注意事項推播。
- **3.6 報修管理 (`maintenance`)**：住戶報修申請、進度追蹤與維修人員派工管理。
- **3.7 訪客管理 (`visitor`)**：訪客預約登記、QR Code 通行證生成與進出紀錄。
- **3.8 社區與住戶資料 (`communityManager`)**：系統管理員用以新增與配置多個社區實體，支援多租戶架構管理。
- **平台核心 (`platform`)**：用於區分與處理 App 端或 Web 端來源的特定邏輯。
...
├─ docker-compose.yml               # Docker 開發環境編排
├─ app/
│  ├─ controller/
│  │  └─ v1/
│  │     ├─ communityManager/       # 3.8 社區資料管理（系統管理員）
│  │     ├─ facility/               # 3.3/3.4 公設管理與預約
│  │     ├─ maintenance/            # 3.6 報修管理
│  │     ├─ message/                # 3.5 訊息中心
│  │     ├─ platform/               # 平台核心邏輯
│  │     ├─ property/               # 3.2 物業後台管理
│  │     ├─ user/                   # 3.1 使用者管理
...
歡迎透過 Issue 或 PR 回報問題、提案，讓 Community Notification System 更臻完善！
