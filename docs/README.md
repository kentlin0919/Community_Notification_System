# 開發文件索引

此目錄依主題分類專案文件，方便快速尋找對應資訊。Swagger 相關產出仍維持在目錄根層，請勿手動修改。

## 架構與流程
- `architecture/router_flow.md`：說明路由註冊與請求處理時序，含登入流程時序圖。

## Commit 摘要
- `commit_summaries/commit_summary_2025_10.md`：2025 年 10 月變更摘要，最新紀錄置於最上方。
- `commit_summaries/commit_summary_2025_09.md`：2025 年 9 月變更摘要。

## Swagger 產出
- `docs.go`、`swagger.json`、`swagger.yaml`：由 `swag init` 產生，更新 API 註解後請重新產生。

> 新增文件時，請將內容依主題放入對應子資料夾，若無合適分類可先建立新資料夾並於本索引登錄。
